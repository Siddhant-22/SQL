...
SELECT SUM(SALARY), DEPARTMENT FROM EMPLOYEE100 GROUP BY DEPARTMENT ORDER BY SUM(SALARY) DESC;
SELECT SUM(SALARY), DEPARTMENT, GENDER FROM EMPLOYEE100 GROUP BY DEPARTMENT, GENDER ORDER BY SUM(SALARY) DESC;
...
#USE COALESCE() TO NAME THE NULL COLUMN
SELECT COALESCE (DEPARTMENT,'ALL DEPARTMENTS') AS DEPARTMENT, SUM(SALARY)
FROM EMPLOYEE100
GROUP BY DEPARTMENT WITH ROLLUP;
...
#CUBE CLAUSE
#CUBE DOES NOT RUN IN MYSQL SO IT WILL BE DONE IN SNOWFLAKE
SELECT 
COALESCE (DEPARTMENT,'ALL DEPARTMENTS') AS DEPARTMENT,
COALESCE (GENDER,'ALL GENDERS') AS GENDER,
SUM(SALARY) AS SALARY_SUM
